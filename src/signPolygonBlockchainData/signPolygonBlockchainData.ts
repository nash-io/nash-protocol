...
// Signing for Polygon needs a little more work to be done.
...
export function signPolygonBlockchainData(privateKey: string, data: string): BlockchainSignature {
...
export async function presignPolygonBlockchainData(
  apiKey: APIKey,
  config: PresignConfig,
  data: string,
  performHash: boolean = true
): Promise<BlockchainSignature> {
  const finalHash = performHash ? createHashedMessage(data).toString('hex') : data
  const polygonChildKey = apiKey.child_keys[BIP44.POLYGON]!
  const { r, presig } = await computePresig({
    apiKey: {
      client_secret_share: polygonChildKey.client_secret_share,
      paillier_pk: apiKey.paillier_pk,
      public_key: polygonChildKey.public_key,
      server_secret_share_encrypted: polygonChildKey.server_secret_share_encrypted
    },
...
export function buildPolygonOrderSignatureData(
  address: string,
  ...
  buffer.writeString(getPolygonAssetID(orderData.source.symbol))
  buffer.writeString(getPolygonAssetID(orderData.destination.symbol))
...
export function buildPolygonMovementSignatureData(address: string, payloadAndKind: PayloadAndKind): string {
  ...
  buffer.writeString(address)
  buffer.writeString(getPolygonAssetID(unitA))
